...
void Client::generateAutoindexResponse(ServerConfig &config) {
...
bool Client::isValidRequest(const ServerConfig &currentConfig, const Location &currentLocation,
...
Client &Client::operator=(const Client &socket) {
    if (this == &socket)
        return *this;
    _socket = socket._socket;
    _clientInterest = socket._clientInterest;
...
bool Client::operator==(const Client &socket) const {
...
void Client::chooseConfig(const std::string &host, ServerConfig &config) {
    for (size_t i = 0; i < _config.size(); i++) {
